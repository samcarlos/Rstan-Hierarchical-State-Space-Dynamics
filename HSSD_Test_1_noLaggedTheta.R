...
parameters {
  ...
  corr_matrix[P*L] Omega;
  vector<lower=0>[P*L] tau;
  vector[N] mu_theta;
  vector[P*L] mu_beta; 
  real<lower=0> sigmasq_theta[N];
  real<lower=0> sigmasq_y[N];
  vector<lower=0>[P*L] sigmasq_beta;
...
transformed parameters {
  ...
  for (n in 1:N)
    for (t in (2+L):T){
      theta[n,t]<-theta[n,t-1] + explanvars[n,t] +sigmasq_theta[n]*theta_innovation[t,n];
    }

}

model {
  matrix[L*P,L*P] Sigma_beta;
  mu_beta ~ normal(0, 1);
...
"
fit <- stan(model_code = model, data = stan.dat, iter = 1000, chains = 1, verbose = TRUE)
